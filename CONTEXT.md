...
|---|---|---|
| **Virtualization** | Ch. 4 (§4.1 process, §4.4 states Running/Ready/Blocked); Ch. 6 (§6.1 direct execution, §6.2 syscalls, §6.3 timer interrupt); Ch. 8 (§8.1-§8.5 MLFQ); **Ch. 9 lottery + stride scheduling**; **Ch. 10 multi-CPU scheduling (SQMS / MQMS / work stealing)**; Ch. 13 (address space, implicit); Ch. 18 (linear page table); **Ch. 19 TLB** (§19.4 ASID — spec-only in M28); **Ch. 20 multi-level page tables**; **Ch. 21 + Ch. 22 replacement policies**; **Ch. 23 complete VM (COW + swapping)**; Ch. 26-27 (thread = point of execution, thread API); Ch. 33 (event-based) | M1, M2, M3, M4, M7, M13, M13.2, M13.3, M14, M16, M17, M18, M19, M28 (spec) |
| **Concurrency** | Ch. 26 (§26.4 figure 26.7 the race); Ch. 27 (thread API); Ch. 28 (§28.1 lock abstraction, §28.7 test-and-set, §28.9 CAS, §28.16 two-phase); Ch. 30 (CVs — spec-only in M29); Ch. 31 (§31.4 bounded buffer, §31.5 reader-writer, **§31.6 dining philosophers**); Ch. 33 (events); **Ch. 32.3 lock-free CAS** (§32.3 deadlock prevention + Banker's — spec-only in M30) | M4, M5, M6, M7, M8, M9, M10, M20, M22, M29 (spec), M30 (spec) |
| **Persistence** | Ch. 36 (I/O devices — TCP receive loop uses kernel async I/O; §36.3-§36.6 device drivers + DMA — spec-only in M34); Ch. 38 (§38.2 independent failure model, §38.3 striping, §38.4 mirroring, §38.7 dedicated parity, §38.8 rotating parity); Ch. 39 (§39.3 open, §39.4 read/write, §39.13 rmdir); Ch. 40 (§40.2 vsfs layout, §40.3 inode, §40.4 directory, §40.5 free space, §40.6 access path, §40.7 caching); **Ch. 41 FFS (cylinder groups, locality, large-file exception, filespan/dirspan)**; Ch. 42.3 (data journaling, recovery, batching, circular log, [Tricky Case: Block Reuse] deferred); **Ch. 43 LFS (segments + imap + checkpoint region + segment summary + cleaner)** (§43.3 segment-size math + §43.12 two-CR alternating writes — spec-only in M31); **Ch. 44 Flash SSDs (read/program/erase + log-structured FTL + mapping table + garbage collection + wear leveling + TRIM)** (§44.9 block-level + hybrid FTL — spec-only in M32); **Ch. 45 Data integrity (XOR/additive/Fletcher checksums + physical ID for misdirected writes + write sequence for lost writes + scrubber)** (§45.8 periodic scrubbing schedule — spec-only in M33) | M1, M2, M3, M11, M12.1–12.7, M21, M24, M25, M26, M27, M31 (spec), M32 (spec), M33 (spec), M34 (spec) |
...
**Coverage**: 33 of OSEP's ~50 chapters have working code in this repo (Ch. 4, 6, 8-10, 13, 18-23, 26-28, 30-33, 36, 38-45, 53-57). Seven additional spec-only milestones (M28 ASID, M29 CV, M30 deadlock prevention, M31 LFS extensions, M32 block-level FTL, M33 scrubbing schedule, M34 device drivers) cover the remaining natural sub-section gaps with overview + slice docs written; no implementation yet. The lab focuses on the **core three pieces** (Virtualization + Concurrency + Persistence) plus Part IV Security; remaining chapters (Ch. 7 process API, Ch. 14-17 base+bound / segmentation, Ch. 24-25, Ch. 29, Ch. 34-35, Ch. 46-52 distributed systems / network files / advanced) are not addressed. §-sub-section coverage noted in each milestone's `overview.md`.
...
- **M28 ASID (spec-only)** — Ch. 19 §19.4 ASID-tagged TLB + per-ASID flush. See `m28-asid/overview.md`.
- **M29 Condition variables (spec-only)** — Ch. 32.2 lost-wakeup bug + bounded-buffer CV primitive. See `m29-cv/overview.md`.
- **M30 Deadlock prevention (spec-only)** — Ch. 32.3 prevention strategies (lock ordering, hold-and-wait prevention, preemption) + Banker's algorithm. See `m30-deadlock/overview.md`.
...
- **M32 Block-level FTL (spec-only)** — Ch. 44 §44.9 block-level + hybrid FTL comparison. See `m32-ssd-extensions/overview.md`.
- **M33 Scrubbing schedule (spec-only)** — Ch. 45 §45.8 periodic scrubber + catch-probability model. See `m33-integrity-extensions/overview.md`.
- **M34 Device drivers (spec-only)** — Ch. 36 §36.3-§36.6 canonical protocol + interrupts + DMA + MMIO. See `m34-device-drivers/overview.md`.
...
- **Part III Persistence**: Ch. 36 device drivers (m11 covers the syscall-level surface, not the interrupt/PIO/DMA story); Ch. 38-45 all have working code in this repo — M12 covers Ch. 40 vsfs + Ch. 42.3 journaling; M21 covers Ch. 41 FFS placement; M24 covers Ch. 38 RAID levels 0/1/4/5; M25 covers Ch. 43 LFS segments + imap + CR + cleaner; M26 covers Ch. 44 SSD FTL + GC + wear tracking; M27 covers Ch. 45 data integrity (checksums + physical ID + write sequence + scrubber). **Part III Persistence is closed.**
...
### Concurrency deep-dive (M28, M29, M30)

- **ASID** (Address Space Identifier, M28, OSEP §19.4): a small integer tag (typically 8 bits) added to each TLB entry identifying which address space the entry belongs to. Replaces the per-context-switch `FlushTlb()` with a per-ASID flush; entries for other ASIDs (or kernel global entries) survive.
- **Lost wakeup** (M29, OSEP §32.2): a race where a producer sets a flag and signals a waiter, but the consumer's check-and-wait sequence interleaves such that the consumer is past the check but not yet on the wait queue when the signal fires. The consumer waits forever. The fix is the **condition variable** primitive, which bundles the check + wait into an atomic step.
- **Mesa vs Hoare semantics** (M29, OSEP §32.2): two flavours of condition-variable wakeup. Mesa (the C# `Monitor` default) lets the waker continue after `Pulse` — the waiter doesn't immediately hold the lock. Hoare semantics transfer the lock to the waiter atomically. Mesa needs the canonical `while (!predicate) Wait()` loop to handle spurious wakeups and the lock-not-held window.
- **Spurious wakeup** (M29): a wakeup from a CV that has no producer-side reason. POSIX allows them; C# `Monitor.Wait` can return spuriously. The defensive idiom is `while (!predicate) { lockObj.Wait(); }` — never `if (!predicate)`.
- **Deadlock** (M30, OSEP §32.3): a state where N threads each hold some locks and wait for others to release theirs, forming a cycle. No thread can make progress.
- **Coffman conditions** (M30, OSEP §32.3): the four preconditions for deadlock — mutual exclusion, hold-and-wait, no preemption, circular wait. Removing any one breaks deadlock.
- **Lock ordering** (M30, OSEP §32.3 prevention): assigning each lock a global numeric ID and requiring every thread to acquire locks in ascending ID order. Breaks circular wait.
...
- **Segment-size cost model** (M31, OSEP §43.3): the rule that an optimal segment size = `M · S` (file-size × bandwidth-to-seek ratio). Too small → cleaner thrashes. Too large → bandwidth wasted on partial writes. M31's `SegmentSizer.OptimalBlocks` computes this from `(bandwidth, avgFile, seek, blockSize)`.
- **Write amplification** (M31): ratio of physical disk writes to logical LFS writes. The cleaner + segment-size choice both drive it. An LFS with a well-sized segment and efficient cleaner should be close to 1×; small segments or low live-block ratios push it above 2×.
- **Two-CR alternating writes** (M31, OSEP §43.12): the recovery technique where two checkpoint regions (CR0 + CR1) sit at fixed offsets on disk; the writer alternates on each flush; recovery reads both and picks the one with the higher sequence number. Avoids the deadlock where the writer can't update the CR because the segment it would have used is being flushed.
...
- **Page-level FTL** (M26, OSEP §44.7): every LBA maps to one physical page; the mapping table has one entry per page. Maximum flexibility, highest RAM cost (1 GB / TB at 4 KB pages).
- **Block-level FTL** (M32, OSEP §44.9): every logical block maps to one physical block; the page offset is preserved. The mapping table is `1 / pagesPerBlock` the size of page-level. Writes that don't fill a block go through a small block-level log buffer.
- **Hybrid (log-block) FTL** (M32, OSEP §44.9): data blocks use page-level mapping once they cross an update-count threshold; metadata blocks stay block-level. The standard production choice — combines the speed of page-level with the RAM cost of block-level on hot data only.
...
- **Scrubbing schedule** (M33, OSEP §45.8): the periodic invocation of the scrubber — every `intervalMinutes`, covering `batchSize` blocks per pass, throttled so it doesn't starve foreground I/O. The trade-off: too rare → corruption spreads before detection; too frequent → I/O overhead.
- **Scrubbing catch probability** (M33, OSEP §45.8): the probability that a per-block failure is caught before it spreads to other blocks. Poisson-arrival model: `1 - e^(-λ · t)` where λ = per-block MTTF and t = mean time between scrubber passes of a given block.

### I/O devices (M34)

- **Device register** (M34, OSEP §36.1): a small piece of memory or I/O port that the CPU reads/writes to communicate with a device. Typical devices expose three: status (read state), command (issue an operation), data (transfer bytes).
- **MMIO** (Memory-Mapped I/O, M34, OSEP §36.6): mapping device registers into the regular address space so the CPU accesses them with normal load/store instructions. The standard on modern hardware (PCIe, ARM SoCs).
- **PIO** (Programmed I/O, M34, OSEP §36.6): the older x86 model — special `in`/`out` instructions reach an isolated I/O address space. We don't model it directly because .NET can't issue PIO; the M34 simulator exposes the access pattern only.
- **DMA** (Direct Memory Access, M34, OSEP §36.5): the device reads/writes main memory directly without going through the CPU. The CPU programs a DMA channel with source/dest/count, returns to other work, and is interrupted when the transfer completes. Burns fewer CPU cycles than PIO for large transfers.
- **Canonical protocol** (M34, OSEP §36.3): the four-step device-driver dance — poll status until ready → write command → poll or wait for interrupt → read data register (PIO) or memory (DMA).
- **Interrupt-driven I/O** (M34, OSEP §36.4): the device raises a hardware signal (IRQ) when it needs the CPU's attention. The CPU stops what it's doing, saves state, runs the interrupt handler, returns. Beats busy-polling for I/O-bound workloads.
