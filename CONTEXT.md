...
- `src/MiniWebServer.Host/Program.cs` contains the whole server loop.

There are no test projects, domain submodules, or ADRs yet.
...
3. Receive a small request buffer from the client.
4. Log the decoded request bytes to the console.
5. Send a fixed `HTTP/1.1 200 OK` plain-text response with body `Hello World!`.
6. Close the client socket.

The server handles one client at a time. It has no routing, no request parsing, no concurrency, no graceful shutdown, and no error handling around socket operations.
...
- The request receive buffer is currently only 3 bytes, so console logging shows only the first bytes of the request.
- Port `8080` is hard-coded.
- Socket operations are synchronous and blocking.
- The infinite loop has no cancellation path.
- The fixed response assumes a `Content-Length` that matches `Hello World!`.
