...
Updated 2026-09-21 (M25 LFS added). Legend: ✅ built + experimented + noted · 🟡 planned · ⬜ future.
...
**Milestones**: M1 ✅–M15 ✅, M13.2 ✅, M13.3 ✅, M16 ✅, M17 ✅, M18 ✅, M19 ✅, M20 ✅, M21 ✅, M22 ✅, M23 ✅ (incl. .1 password + .2 at-rest + .3 RBAC + .4 PK + .5 handshake + .6 TOTP), M24 ✅ (RAID 0/1/4/5 with XOR recovery), M25 ✅ (LFS segments + imap + CR + cleaner). M8–M15 are the post-roadmap extensions per `docs/adr/0004-extend-broad-concurrency-roadmap.md` (M8–M12), `docs/adr/0006-mini-scheduler-mlfq-proportional-multicpu.md` (M13), `docs/adr/0007-mini-pager-mmu-tlb-multilevel-replacement.md` (M14). The original 12-slice roadmap is closed. **Latest**: M25 (LFS — closes OSEP Ch. 43 in the persistence thread). See `docs/adr/0008`–`0015` for ADRs. Each yielded a small slice with its own overview + smoke trace in `docs/learning/`.
**Tests**: 30 passing (14 pre-existing + 10 RAID + 6 LFS).
...
**Latest commit**: M25 (LFS — closes OSEP Ch. 43). New file `src/MiniWebServer.Host/MiniScheduler/Lfs.cs` (~480 lines): a single `Lfs` simulator with the full write path — in-memory segment buffer accumulates `LfsBlock`s (Data, Inode, Imap, Summary, Checkpoint) until half-full, then flushes to the next free disk segment. Reads go through the imap + inode chain (`name → inodeNum → imap[ino] → inode → data block`). Segment summary records `(inode, offset)` per block per §43.10; `IsLive(seg, slot)` compares against the current imap + inode's data-pointer to detect dead blocks. Cleaner picks the coldest segment (fewest live blocks), compacts live blocks into a new segment, frees the old one. New HTTP route `/lfs/run?scenario=create|rewrite|clean&segments=N&blocks=M&files=K`. Build clean, 30/30 tests pass (14 pre-existing + 10 RAID + 6 LFS).
...
- **Part III Persistence**: Ch. 36 device drivers (m11 covers the syscall-level surface, not the interrupt/PIO/DMA story); Ch. 44 flash, Ch. 45 data integrity — M12 covers Ch. 40 vsfs + Ch. 42.3 journaling; M21 covers Ch. 41 FFS placement; M24 covers Ch. 38 RAID levels 0/1/4/5; M25 covers Ch. 43 LFS segments + imap + CR + cleaner
...
All twelve roadmap slices + post-roadmap extensions + the VM paging chain + the multi-CPU/dining/FFS/lock-free quartet + the M23 Part IV suite (.1 password + .2 at-rest + .3 RBAC + .4 PK + .5 handshake + .6 TOTP) + M24 (RAID 0/1/4/5) + M25 (LFS segments + imap + CR + cleaner) have learning notes with smoke-test output captured inline. M8 (the first post-roadmap extension) has its own learning note and a 6.3 section appended to the M6 note. The M16-M19 chain + M13.2 + M13.3 + M20 + M21 + M22 + M23 + M24 + M25 all have an `overview.md` and a slice doc under their own folder.
