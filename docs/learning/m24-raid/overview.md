...
- **RAID 0** (OSEP §38.3): block-level striping across `N` disks (no redundancy — `N`× throughput but `N`× failure rate).
- **RAID 1** (OSEP §38.4): full mirroring on `2` disks (writes doubled; survives `1` of `2` failures).
- **RAID 4** (OSEP §38.7): block-level striping + dedicated parity disk (parity disk is the write bottleneck — every write hits it).
- **RAID 5** (OSEP §38.8): block-level striping + rotating parity (parity writes spread across all disks — no bottleneck).
...
- **Ch. 38 Redundant Arrays of Inexpensive Disks** (§38.2 interface + fault model; §38.3 RAID 0 striping; §38.4 RAID 1 mirroring; §38.7 RAID 4 dedicated parity; §38.8 RAID 5 rotating parity).
- §38.5 RAID 2 (bit-level Hamming) and §38.6 RAID 3 (byte-level striping + parity) deferred — superseded by block-level striping in practice.
- §38.9 RAID 6 (dual parity) deferred — extends RAID 5's XOR to a 2D parity (P + Q).
...
| §38.3 RAID 0 striping | Round-robin across `N` disks; stripe unit = one block. | Chunked striping with chunk size > 1 block. |
| §38.4 RAID 1 mirroring | Full duplicate on `2` disks. Reads pick first surviving copy. | >2 mirrors, asymmetric read preference. |
| §38.7 RAID 4 dedicated parity | Parity on a fixed disk (disk `N-1`). Single-write case is the bottleneck demo. | Hot-spare swap. |
| §38.8 RAID 5 rotating parity | Parity stripes rotate: stripe `i`'s parity lives on disk `(i + offset) % N`. | I/O scheduling (e.g., shortest-seek-first placement). |
...
| §38.5/§38.6 RAID 2/3 | Not implemented. | Bit- or byte-level striping. |
| §38.9 RAID 6 | Not implemented. | Dual parity (P+Q). |

## Key OSEP quotes

> "When you build a system, you should make it work, and then make it work better." (OSEP §38.4 TIP) — the chapter builds RAID 0 first, then adds redundancy, then optimizes the bottleneck.

> "The fundamental insight is the XOR operation: XOR of all bits in a row of bits (i.e., the parity) equals 0 if there are an even number of 1s and 1 if odd. ... If we XOR a value into the parity, the parity becomes incorrect. To recompute, we can just XOR out the old value and XOR in the new one." (OSEP §38.7)

> "In RAID 5, the parity is rotated across the disks in the array, so each disk eventually becomes the parity disk. ... This solves the small-write problem." (OSEP §38.8)
...
- XOR recovery: a `BlockAt(stripe, diskIndex)` helper that, on a failed disk, XORs the other `N-1` blocks in the stripe (OSEP §38.7 + §38.8 "to recompute parity, you can just XOR").
