# Milestone 30: Deadlock Prevention & Avoidance (Ch. 32.3)
...
Plus a **Banker's algorithm** demo: given N threads and M resource classes, the banker grants an allocation request only if the resulting state is safe (every thread can still complete).
The slice drives these from `/deadlock/run?scenario=...` and reports which strategy fixed (or failed to fix) the deadlock.
## Slice
...
## OSTEP coverage
- **Ch. 32 §32.3** "Preventing Deadlock" [SH92]: cycle prevention (lock ordering), hold-and-wait prevention (atomic batch acquire), preemption (try-lock + release-all on timeout), and the practical argument that prevention wastes resources / is rarely used.
- **Ch. 31 §31.6** "Dining Philosophers" [CM72] — M20 built the problem; M30 is the prevention half.
- **Ch. 33** (deferred): detection-and-recovery via wait-for graphs is a different school of thought; the slice focuses on §32.3 prevention.
## Files
...
- **No detection-and-recovery** (OSEP Ch. 33): the textbook's other approach. The slice is purely on prevention, matching §32.3.
- **Single-machine locks only**: distributed deadlock (Ch. 33's transactional memory analog) is out of scope.
## What this slice does NOT do
- **Wait-for graph + cycle detection** (Ch. 33) — the runtime-detection school.
