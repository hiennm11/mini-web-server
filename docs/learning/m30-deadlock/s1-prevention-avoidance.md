...
- `DeadlockSim` (new class in `MiniScheduler`):
  - `void AcquireAll(Lock[] locks)` — atomic batch acquire; holds a meta-monitor while taking each lock in turn. Prevents **hold-and-wait** (Coffman condition 2).
  - `bool AcquireWithTimeout(Lock l, TimeSpan timeout)` — try-lock + retry; on timeout, releases everything held. Prevents **no preemption** (Coffman condition 3).
  - `void AcquireInOrder(Lock[] locks)` — sorts by lock ID and takes in ascending order. Prevents **circular wait** (Coffman condition 4). Mirrors OSEP §32.3 TIP "ENFORCE LOCK ORDERING BY LOCK ADDRESS".
...
- `tests/MiniWebServer.Host.Tests/Program.cs` adds:
  ...
  - `Run("banker rejects unsafe request", ...)` — assert Banker rejects the canonical unsafe allocation from OSEP §32.3.
...
- OSEP Ch. 32 §32.3 — four Coffman conditions + prevention strategies + Banker's algorithm + detect-and-recover.
