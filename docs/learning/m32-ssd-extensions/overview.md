# Milestone 32: Block-Level FTL (§44.9)
...
1. **Page-level FTL** (M26 already has this): every logical page maps to one physical page. Mapping table = `O(capacity_in_pages)` — the OSEP §44.9 "impractical for 1 TB devices" case.
2. **Block-level FTL** (this slice): every logical block maps to one physical block; inside a block, the offset is preserved. Mapping table = `O(capacity_in_blocks)` — `1 / pagesPerBlock` smaller.
3. **Hybrid (log-block) FTL**: data blocks use page-level mapping (high-performance for hot data), metadata blocks use block-level mapping (low-cost for cold data). The switch point is a per-block update count threshold.
...
## OSTEP coverage
- **Ch. 44 §44.9 "Block-Level Mapping"** + §44.9 hybrid mapping: the canonical mapping-strategy trade-off, taught as the answer to "why isn't every SSD page-mapped?".
...
- **Hybrid switch point is per-block update count ≥ 4** (matching OSEP §44.9 figure 44.10). Real drives tune this empirically.
