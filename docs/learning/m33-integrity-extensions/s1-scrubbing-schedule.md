# Slice 33.1: Scrubbing Schedule (Ch. 45 §45.7)
...
- `IntegrityStore` (extended in `MiniScheduler`):
  ...
  - `static double ProbabilityOfCatch(int intervalMinutes, int batchSize, int totalBlocks, double blockMtbfHours)` — the catch-probability model.
- The comparison route reports per-config:
  - `blocksCoveredPerPass`
  - `meanTimeBetweenPasses` (interval × totalBlocks / batchSize, accounting for sweep coverage)
  - `probabilityOfCatchBeforeSpread` (the Poisson model)
## Files
...
- `/integrity/run?scenario=scrub-schedule&interval_minutes=1440&batch_size=100&block_mtbf_hours=100000` reports a measurably lower catch probability (the OSEP §45.8 "too rare" case).
...
- `tests/MiniWebServer.Host.Tests/Program.cs` adds:
  ...
  - `Run("scrub probability matches Poisson model", ...)` — assert `ProbabilityOfCatch(I, B, N, MTTF)` matches a hand-calculated value for known parameters.
...
- OSEP Ch. 45 §45.7 — periodic scrubbing + the "nightly or weekly" scheduling baseline.
- OSEP Ch. 45 §45.8 — space/time overheads argument that motivates the schedule parameters.
