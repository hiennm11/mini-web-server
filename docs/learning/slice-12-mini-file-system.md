...
- **Atomic `unlink` of directories** (`rmdir`): same wrapping needed for `Idestroy` of a directory. Currently `UnlinkFile` only handles files; directories would need a similar treatment.
